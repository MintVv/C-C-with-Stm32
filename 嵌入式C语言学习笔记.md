...
### 全局变量、`static` 局部变量与普通局部变量

题目：判断下面变量能否在 `main()` 中直接访问。
...
- `global_value`：能在同一个 `.c` 文件的 `main()` 中访问；
- `static_value`：不能，因为它的作用域只在 `test()` 函数内部；
- `local_value`：不能，因为它只在本次 `test()` 函数调用期间有效。

对比：

| 变量类型 | 作用域 | 生命周期 |
| --- | --- | --- |
| 全局变量 | 当前 `.c` 文件中定义位置之后；可用 `extern` 在其他文件声明访问 | 整个程序运行期间 |
| `static` 局部变量 | 当前函数内部 | 整个程序运行期间 |
| 普通局部变量 | 当前函数或代码块内部 | 进入代码块时创建，离开时结束 |

### 文件作用域的 `static`

题目：判断两个源文件之间能否通过 `extern` 访问 `static` 全局变量。
...
文件作用域的 `static` 表示该变量只能在声明它的那个 `.c` 文件内部使用，其他文件即使写 `extern int temperature;` 也不能访问。链接时会出现未定义的符号错误。

用途：

- 限制内部变量或函数只在当前 `.c` 文件可见，避免多个文件互相干扰；
- 实现“模块私有”的数据和函数，例如传感器内部校准参数、底层驱动函数。

对比：

- 文件作用域 `static` 全局变量：**当前 `.c` 文件内部可见**；
...
题目：`for` 循环从下标 4 递减到 0，输出顺序和循环结束后的 `i` 值。

```c
int data[5] = {5, 3, 9, 1, 7};
...
原因：第一次以 `i = 4` 访问最后一个元素 `7`，一直执行到 `i = 0` 访问第一个元素 `5`。

`i` 最终的值为 `-1`：

- 当 `i = 0` 时，条件 `0 >= 0` 成立，执行循环体；
- 执行完循环体后执行 `i--`，`i` 变成 `-1`；
- 再次检查条件 `-1 >= 0` 不成立，循环结束。

### `do-while` 至少执行一次

题目：`do-while` 执行次数和循环结束后的值。
...
| 次数 | 执行前 count | sum 变化 | 执行后 count | 判断 |
| --- | --- | --- | --- | --- |
| 1 | 0 | 0 + 0 | 1 | `1 < 3` 成立 |
| 2 | 1 | 0 + 1 | 2 | `2 < 3` 成立 |
| 3 | 2 | 1 + 2 | 3 | `3 < 3` 不成立，结束 |

要点：

- `do-while` 先执行一次循环体，再判断条件；
- `while` 是先判断条件，可能一次都不执行；
- 累加时注意 `sum = sum + count` 使用的仍是加 `1` 之前的 `count`，所以结果是 `0 + 1 + 2 = 3`，而不是 `1 + 2 + 3 + 4`。
...
- 循环结束时 `i = 3`；
- `break` 的作用是立即结束当前循环，执行循环之后的代码。

分析：

- 当 `i = 3` 时，`if (i == 3)` 成立，执行 `break`，结束 `while(1)`；
- 如果没有 `break`，`while(1)` 会一直循环下去；
- `break` 只作用于当前循环，不会影响外层循环；
- STM32 主循环常用 `while (1)`，通过开启的模块和中断完成持续运行。
